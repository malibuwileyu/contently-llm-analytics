...
export class CacheWarmupService {
  ...
  constructor(
    ...
    this.enabled = this.configService.get<boolean>('cache.warmup.enabled', true);
    this.timeout = this.configService.get<number>('cache.warmup.timeout', 30000);
  }
      ...
      try {
        await this.lockService.withLock(
          ...
          async () => {
            ...
            this.logger.debug(`Successfully warmed up cache for: ${provider.key}`);
          },
          { ttl: this.timeout }
        );
      } catch (error) {
