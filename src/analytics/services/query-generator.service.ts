...
export class QueryGeneratorService {
  ...
  async generateQueriesForType(
    ...
    while (queries.size < count && attempts < maxAttempts) {
      ...
      const variables =
        this.variableBankService.generateRandomVariablesForTemplate(
          template.variables || [],
        );
  ...
  async generateQueriesForAllTypes(
    countPerType: number,
  ): Promise<Record<QueryType, string[]>> {
    const types: QueryType[] = [
      QueryType.INDUSTRY,
      QueryType.CONTEXT,
      QueryType.COMPETITIVE,
    ];
    const result: Record<QueryType, string[]> = {} as Record<
      QueryType,
      string[]
    >;
  ...
  async generateValidatedQueries(
    type: QueryType,
    count: number,
  ): Promise<string[]> {
    ...
    while (validQueries.length < count && attempts < maxAttempts) {
      const queries = await this.generateQueriesForType(
        type,
    ...
    if (validQueries.length < count) {
      throw new Error(
        `Could not generate enough valid queries for type: ${type}`,
      );
    }
