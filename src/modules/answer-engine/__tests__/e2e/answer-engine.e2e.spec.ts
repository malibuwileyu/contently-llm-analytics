...
describe('AnswerEngine E2E Tests', () => {
  ...
  beforeEach(async () => {
    ...
    mainRunnerService.runOne.mockImplementation(async (name, context) => {
      if (name !== 'answer-engine') {
        return {
          success: false,
          error: { message: 'Runner not found', timestamp: new Date() },
        };
      }
  ...
  describe('Service Tests', () => {
    it('should analyze content', async () => {
      ...
      expect(sentimentAnalyzerService.analyzeSentiment).toHaveBeenCalledWith(
        mockContent,
      );
  ...
  describe('Runner Integration', () => {
    ...
    it('should handle errors in the runner', async () => {
      ...
      sentimentAnalyzerService.analyzeSentiment.mockRejectedValueOnce(
  ...
  describe('Performance and Error Handling', () => {
    it('should handle concurrent requests', async () => {
      ...
      const promises = Array(5)
        .fill(0)
        .map(() => answerEngineService.analyzeMention(dto));
