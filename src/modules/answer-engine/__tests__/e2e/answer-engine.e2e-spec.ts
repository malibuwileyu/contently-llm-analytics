...
describe('Answer Engine E2E', () => {
  ...
  const testContent = 'This is a positive review of the product. It works great and I love it!';
  const testCitations = [
    { source: 'https://example.com/review1', text: 'Great product', metadata: { page: 1 } },
    { source: 'https://academic.edu/paper', text: 'Scientific analysis', metadata: { section: 'Results' } }
  ];
  
  beforeAll(async () => {
    // Create a test database module with the required entities
    const testDbModule = await createTestDatabaseModule([BrandMention, Citation]);
    ...
    answerEngineService = moduleRef.get<AnswerEngineService>(AnswerEngineService);
  ...
  describe('End-to-end flow', () => {
    it('should process content through the MainRunnerService', async () => {
      ...
      if (academicCitation && regularCitation) {
        expect(academicCitation.authority).toBeGreaterThan(regularCitation.authority);
      } else {
      ...
      const contexts = [
        {
          ...
          metadata: {
            ...
            context: { query: 'review 1', response: 'Full response 1', platform: 'test-platform' },
        ...
        },
        {
          ...
          metadata: {
            ...
            context: { query: 'review 2', response: 'Full response 2', platform: 'test-platform' },
        ...
        },
        {
          brandId: testBrandId,
          metadata: {
            content: 'This is a negative review. The product did not meet expectations.',
            context: { query: 'review 3', response: 'Full response 3', platform: 'test-platform' },
    ...
    it('should handle errors gracefully', async () => {
      ...
      const result = await mainRunnerService.runOne('answer-engine', invalidContext);
    ...
    it('should process multiple mentions efficiently', async () => {
      ...
      const contexts = Array.from({ length: 10 }, (_, i) => ({
        ...
        metadata: {
          ...
          context: { query: `query ${i}`, response: `response ${i}`, platform: 'test-platform' },
          citations: [{ source: `https://example.com/${i}`, text: `Citation ${i}` }]
        }
      }));
      ...
      console.log(`Processed 10 mentions in ${duration}ms (${duration / 10}ms per mention)`);
