...
export class AuthController {
    ...
    try {
      ...
      if (this.DISALLOWED_DOMAINS.includes(emailDomain)) {
        throw new HttpException({
          ...
          message: 'Please use a valid email address. Test domains are not allowed.',
          details: {
            email: 'Test email domains (example.com, test.com) are not allowed'
          }
      ...
      if (error) {
        ...
        if (error.message?.includes('Email address') && error.message?.includes('is invalid')) {
          throw new HttpException({
            status: HttpStatus.BAD_REQUEST,
            error: 'Invalid email',
            message: 'Please provide a valid email address that can receive verification emails.',
            details: {
              email: 'The email address provided cannot receive verification emails. Please use a real email address.'
            }
          }, HttpStatus.BAD_REQUEST);
        }
  ...
  async googleAuthRedirect(@Req() req: Request) {
    ...
    const { data, error } = await this.supabaseService.signInWithGoogle(googleUser);
