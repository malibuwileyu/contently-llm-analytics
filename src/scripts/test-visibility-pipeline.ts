...
async function testVisibilityPipeline() {
  ...
  try {
    ...
    const competitorRepo = app.get('CompetitorEntityRepository') as Repository<CompetitorEntity>;
    const industryRepo = app.get('IndustryEntityRepository') as Repository<IndustryEntity>;
    ...
    if (!company) {
      company = await competitorRepo.save({
        ...
        key_products: ['Content Marketing Platform', 'Content Strategy', 'Content Analytics'],
      });
    }
    ...
    const queries = await contextQueryService.generateQueriesForCompany(company, queryConfig);
    ...
    for (const [type, responses] of Object.entries(testResponses)) {
      ...
      for (const [i, response] of responses.entries()) {
        ...
        logger.log('Metrics:', {
          ...
          prominenceScore: result.prominence_score,
        ...
        if (result.context_data) {
          logger.log('Context Data:', result.context_data);
        }
      }
    }
    ...
    const trends = await visibilityAnalyticsService.getVisibilityTrends(company, {
      start: weekAgo,
      end: now
    });
    ...
    const outputPath = path.join(__dirname, '../../visibility-test-results.json');
