...
export const queryTemplates: QueryTemplate[] = [
      ...
      requiredElements: [
        {
          type: 'fact',
          description:
            'Must contain specific company names and their offerings',
        },
        {
          type: 'metric',
      ...
      qualityChecks: [
        {
          type: 'specificity',
          validator: response => {
            const companyMentions = response.match(
  ...
  // Context Queries
  {
    ...
    template:
      'How does {company_name} differentiate its {product/service} through {approach} to serve {target_segment}?',
    variables: [
      'company_name',
      'product/service',
      'approach',
  ...
  // Competitive Queries
  {
    ...
    template:
      'Compare {company_name} with other {solution_type} providers in terms of {aspect}',
      ...
      qualityChecks: [
        {
          type: 'specificity',
          validator: response => {
            const competitors = response.match(
              /\b[A-Z][a-zA-Z]*(?:\s+[A-Z][a-zA-Z]*)*\b/g,
            );
