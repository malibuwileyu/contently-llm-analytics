...
async function generateQuestions(
  ...
  // Add freeform type
  const queryTypes = ['industry', 'context', 'freeform'];
  ...
  logger.log(`Generated ${questionsByType['freeform'].length} freeform questions`);
...
async function processCustomerAnalytics() {
            ...
            `, [
              ...
              JSON.stringify({
                ...
                analysis.brandMentions.reduce((sum, mention) => 
                  sum + mention.knowledgeBaseMetrics.authorityScore, 0) / analysis.brandMentions.length,
                (analysis.brandMentions[0].visibility.contextScore + 
                 analysis.brandHealth.visibilityMetrics.overallVisibility) / 2
              ]);
